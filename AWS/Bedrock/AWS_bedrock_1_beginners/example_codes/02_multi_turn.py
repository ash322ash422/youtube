...
def multi_turn_conversation():
    ...
    system_prompt = [
        {
            "text": ("You are an expert AWS Certified Solutions Architect "
                      "Professional and an elite technical instructor.")
        }
    ]
    ...
    # Turn 1: Ask for academic suggestions
    print("\nTurn 1: User asks for academic related suggestions")
    print("-" * 60)

    user_message_1 = ("I am writing a master's thesis on genomics "
                      "and have a 500GB dataset. What AWS storage "
                      "and analytics tools should I use to process "
                      "this on a student budget? .")
    ...
    try:
        response_1 = bedrock_client.converse(
            modelId=model_id,
    ...
    user_message_2 = (
        "That makes sense. If my university "
        "requires all raw patient data to remain "
        "strictly within European boundaries "
        "for compliance, how does that change the architecture?"
    )
    ...
    try:
        response_2 = bedrock_client.converse(
            modelId=model_id,
            system=system_prompt,
            messages=conversation_history,
            inferenceConfig={"temperature": 0.7, "maxTokens": 200}
    ...
    # Turn 3: Ask for cost information
    print("\n\nTurn 3: User asks for cost information")
    print("-" * 60)

    user_message_3 = "How much would this cost me ?"
    ...
    try:
        response_3 = bedrock_client.converse(
            modelId=model_id,
            system=system_prompt,
            messages=conversation_history,
            inferenceConfig={"temperature": 0.7, "maxTokens": 200}
