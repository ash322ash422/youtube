...
# To run this as standalone: python -m app.services.tender_export_service
...
from app.services.tender_extraction_service import extract_data
...
if __name__ == "__main__":
    # Minimal end-to-end smoke test: reads a cached OCR file, runs the real
    # LLM extraction, and writes the resulting workbook - no hand-built
    # sample data needed to sanity-check the Excel writer itself.
    from app import config

    filename = "03_tender_mini_version.ocr.json"
    path = config.CACHE_DIR / filename
    with open(path, "r", encoding="utf-8") as f:
        documents = json.load(f)

    pages = documents["pages"]
    extraction = extract_data(pages)

    out = export_extraction_to_excel(extraction, "03_tender_mini_version.xlsx")
    print(f"wrote {out}")
